 ...
 * `learn_merchant_memory` server-side), and the category/label managers (a
 * delete/rename changes which rules are live / their joined names). Without it,
 * with `staleTime: 30_000` + `refetchOnWindowFocus: false`, those surfaces serve
 * a stale list and rule actions can 404 on a now-deleted id.
 *
